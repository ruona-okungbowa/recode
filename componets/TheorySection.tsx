      ...
      {analogies.length > 0 && (
        <View className="mb-6">
          {analogies.map((analogy, index) => (
            <View
              key={index}
              className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4 mb-3"
            >
              <Text className="text-purple-400 text-xs font-semibold mb-2">
                💡 REAL-WORLD ANALOGY
              </Text>
              <Text className="text-gray-300 text-sm leading-5">{analogy}</Text>
            </View>
          ))}
        </View>
      )}
          ...
          {codeExamples
            ...
            .map((example, index) => (
              <View key={index} className="mb-4">
                <View className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-2">
      ...
      {diagrams.length > 0 && (
        <View className="mb-6">
          ...
          {diagrams.map((diagram, index) => (
            <View
              key={index}
              className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-3 items-center"
            >
